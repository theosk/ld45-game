...
BEGIN
    ...
    state_dialog_clear();
    dialog_putline("You enter the jewel store.", 400);
    dialog_putline("They only sell wedding rings. Just one model.", 400);

    if (quests.saveMerchant == STATUS_FAILED)
        script_place_jewel_ship_merchant_dead_text();
    elseif (inventory.weddingRing)
        dialog_putline("You already have one. Let's just leave.", 400);
...
BEGIN
    dialog_putline("But there's nothing to buy and no one is here.", 400);
    dialog_putline("The shop owner was that merchant you left to die in the", 400);
    dialog_putline("woods. That means no rings for you.", 400);
    dialog_putline("", 400);
    state_dialog_ask("Are you happy now?");
    dialog_putline("Well, I gues it doesn't matter now.", 400);
...
BEGIN
    dialog_putline("The wing costs " + itoa(shops.price_ring) + " coins.", 400);
    dialog_putline("It gives you an extra point on attack rolls", 400);
    state_dialog_ask("Will you buy it?");
    if(dialog.chosen_answer == 1)
        inventory.weddingRing = 1;
        inventory.gold -= shops.price_ring;
        dialog_putline("You got a RING! Your attacks gain 1 point. Why not?.", 400);
...
BEGIN
    dialog_putline("The ring costs " + itoa(shops.price_ring) + " coins.", 400);
